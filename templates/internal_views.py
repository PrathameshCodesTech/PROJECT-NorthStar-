...
class InternalMigrateTenantView(APIView):
    ...
    def post(self, request):
        """Run company_compliance migrations on tenant database"""
        ...
        if not tenant_slug or not connection_name:
            return Response(
                {'error': 'tenant_slug and connection_name required'}, 
        ...
        try:
            ...
            response = requests.get(
                ...
                headers={'X-Internal-Token': 'service2-secret'},
                timeout=10
            )
            
            if response.status_code != 200:
                return Response(
                    {'error': 'Could not get tenant credentials from Service 2'}, 
                    status=status.HTTP_400_BAD_REQUEST
                )
...
class InternalDistributeTemplatesView(APIView):
    ...
    def post(self, request):
        """Copy framework templates to tenant database"""
        ...
        if not tenant_slug:
            return Response(
                {'error': 'tenant_slug required'}, 
                status=status.HTTP_400_BAD_REQUEST
            )
        
        try:
            # Get tenant database credentials and register alias
            response = requests.get(
                f'http://localhost:8001/api/v2/internal/tenants/{tenant_slug}/credentials/',
                headers={'X-Internal-Token': 'service2-secret'},
                timeout=10
            ...
            result = copy_framework_templates_to_tenant(
                tenant_slug=tenant_slug,
